...
const ZooplaProperties = () => {
    ...
    useEffect(() => {
       ...
       const getProperties = async () => {
           try {
               const { data } = await axios.post('https://breadbox-app-api.herokuapp.com/api/v1/areaProperties', {area: defaultValue} )
    ...
    const handleSearch = async () => {
        setLoading(true)
        try {
            const { data } = await axios.post('https://breadbox-app-api.herokuapp.com/api/v1/areaProperties', {area: area} )
            setProperties(data.properties)
            setLoading(false)
            setErrorData({})
